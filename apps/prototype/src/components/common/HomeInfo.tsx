...
const HomeInfo: React.FC = () => {
  return (
    <div className="flex gap-6 p-6 bg-background border border-border rounded-2xl shadow-xl max-w-3xl mx-auto">
      {cards.map((card) => (
        <motion.button
          key={card.title}
          whileHover={{ scale: 1.045, boxShadow: '0 6px 24px 0 rgba(0,0,0,0.18)' }}
          whileTap={{ scale: 0.97 }}
          className="flex flex-col items-start w-64 bg-transparent border-none outline-none cursor-pointer focus:ring-1 focus:ring-yellow-500 rounded-xl p-4 hover:bg-card transition-colors duration-200"
          tabIndex={0}
        ...
        >
          ...
          <h3 className="w-full text-base font-normal text-foreground mb-1 text-left">{card.title}</h3>
          <p className="w-full text-xs font-normal text-muted-foreground text-left">{card.description}</p>
        </motion.button>
      ))}
    </div>
  );
